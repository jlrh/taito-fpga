...
module opwolf_tilemap #( parameter
    RAM_MSB  = 0
)(
    input           rst,
    input           clk,
...
// OFFSETS medidos contra el golden (validado 0.00% vs MAME) con la escena 1200:
// con los de rastan (hrdout -5, vf +8) la imagen salia 5 px a la IZQUIERDA y 1 linea ABAJO.
//   hrdout -10 (era -5): mueve la imagen 5 px a la DERECHA
//   vf     + 9 (era +8): la sube 1 linea
always @* begin
    hscan = hcnt[8:3] + ~hpos[8:3] + 6'd2;   // +2 tiles = +16 px, casa con el +16 del golden
    hrdout = hdump -9'd10;
    vf    = flip ? vdump-9'd240 : vdump+9'd9;
    vscan = vf  + ~vpos;
end
